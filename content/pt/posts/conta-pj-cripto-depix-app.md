---
title: "Conta PJ cripto sem burocracia com DePix App"
description: "Descubra como comerciantes, empresários e autônomos podem aceitar pagamentos via Pix e receber em cripto com o DePix App."
date: 2026-03-30
tags: ["conta pj", "comerciantes", "pagamentos", "depix", "cripto"]
author: "DePix"
slug: "conta-pj-cripto-depix-app"
translationKey: "crypto-business-account"
image: "/images/posts/crypto-business-account/crypto-business-account-16x9.png"
images:
  - "/images/posts/crypto-business-account/crypto-business-account-1x1.png"
  - "/images/posts/crypto-business-account/crypto-business-account-4x3.png"
  - "/images/posts/crypto-business-account/crypto-business-account-16x9.png"
draft: false
---

Se você é comerciante, empresário ou autônomo, já sabe: abrir uma conta PJ no banco é sinônimo de burocracia, papelada e espera. E mesmo depois de abrir, você fica sujeito a bloqueios, taxas escondidas e limites que não fazem sentido para o seu negócio. E se existisse uma alternativa que aceita Pix, não exige CNPJ e entrega seu dinheiro direto na sua carteira digital? É exatamente isso que o DePix App oferece com o modo lojista.

Neste artigo, vamos mostrar como o DePix App funciona como uma conta PJ cripto sem burocracia — e por que faz sentido para quem quer mais controle sobre o próprio dinheiro.

## O problema das contas PJ tradicionais

Abrir uma conta PJ em qualquer banco brasileiro exige CNPJ, contrato social, comprovante de endereço comercial e uma pilha de documentos. O processo pode levar dias ou semanas, e muitas vezes o banco ainda pede que você vá presencialmente a uma agência.

Depois de abrir, os problemas continuam. Bancos podem bloquear contas por motivos administrativos, congelar fundos por ordem judicial ou simplesmente impor limites de movimentação que atrapalham o fluxo de caixa. Se você é um pequeno comerciante ou autônomo, essas restrições podem ser devastadoras.

E as maquininhas de cartão? Além das taxas por transação, você precisa comprar ou alugar o equipamento, pagar mensalidades e lidar com prazos de recebimento que podem chegar a 30 dias no caso de vendas parceladas.

Para quem está começando ou quer simplicidade, esse modelo tradicional não faz sentido. Se você quer entender melhor a diferença entre o sistema bancário tradicional e o DePix, confira nosso artigo sobre [DePix vs Pix tradicional](/posts/depix-vs-pix-tradicional/).

## Comparando custos: DePix vs maquininha

Vamos ser diretos sobre custos. O DePix App cobra **2% + R$0,99 por transação**¹. Não existe mensalidade, não existe aluguel de equipamento e não existe taxa escondida.

Compare com as maquininhas tradicionais:

| Método | Taxa por transação | Custos extras |
|---|---|---|
| **DePix App** | 2% + R$0,99 | Nenhum |
| **Débito (maquininha)** | 0,59% a 1,99%² | Aluguel do equipamento + mensalidade |
| **Crédito à vista** | 3,19% a 4,99%² | Aluguel + mensalidade |
| **Crédito parcelado (12x)** | 8,99%+² | Aluguel + mensalidade + prazo longo de recebimento |

A taxa do DePix é competitiva com as taxas de débito quando você considera que não precisa de nenhum hardware — só um celular. E é **muito mais barata** do que qualquer taxa de cartão de crédito. Para autônomos e pequenos comerciantes que atendem principalmente com Pix e débito, a conta fecha.

Outro ponto importante: com maquininha, você ainda depende de um banco para receber. Com o DePix, seu dinheiro vai direto para a sua carteira Liquid, sem intermediários bancários.

## Como funciona o modo lojista do DePix App

O DePix App tem um modo específico para comerciantes e autônomos: o **"Sou lojista ou autônomo"**. O processo é simples:

{{< diagram alt="Fluxo de pagamento para lojistas no DePix App: cliente paga via Pix normal, verificação anti-fraude D+1, DePix depositado na carteira do lojista com opção de saque ou conversão em Bitcoin" caption="Pagamento no modo lojista: do Pix do cliente até a carteira Liquid do comerciante." >}}
<div class="d-flow-row">
  <div class="d-step muted">Cliente (Pix)</div>
  <div class="d-arrow"></div>
  <div class="d-step accent">DePix App Lojista</div>
  <div class="d-arrow">D+1</div>
  <div class="d-step warn">Verificação</div>
  <div class="d-arrow"></div>
  <div class="d-step success">Carteira do Lojista</div>
</div>
{{< /diagram >}}

1. **Crie sua conta** no [depixapp.com](https://depixapp.com/) — leva menos de um minuto. Basta um e-mail. Sem KYC, sem documentos, sem CNPJ.
2. **Baixe uma carteira Liquid** como a SideSwap e obtenha seu endereço Liquid.
3. **Cadastre seu endereço Liquid** no DePix App.
4. **Gere QR codes Pix** para suas vendas — cada QR code corresponde ao valor da venda.

Quando seu cliente paga, ele usa o Pix normalmente, direto do banco dele. O cliente não precisa saber nada sobre cripto, não precisa de carteira digital, não precisa de nada diferente do que já faz no dia a dia. Para ele, é um Pix comum.

Do seu lado, o valor é convertido em DePix (uma stablecoin pareada ao real na rede Liquid) e enviado para a sua carteira. O valor mínimo por transação é R$5 e o máximo é R$6.000 por dia por CPF¹.

Se você quer entender melhor [o que é DePix](/posts/o-que-e-depix/) e como a stablecoin funciona, temos um artigo dedicado ao assunto.

## Soberania financeira para o seu negócio

Essa é a parte que muda o jogo. Quando você recebe via DePix, seu faturamento vai para a **sua** carteira Liquid — não para uma conta bancária que o banco controla.

Isso significa:

- **Sem risco de bloqueio bancário.** Ninguém pode congelar seus fundos por motivo administrativo.
- **Sem bloqueios judiciais** na sua conta de recebimento. Seus DePix estão na sua carteira, sob seu controle.
- **Proteção contra inflação.** Converta seus DePix para USDt (dólar digital) direto na rede Liquid e proteja seu poder de compra.
- **Poupança em Bitcoin.** Converta parte do seu faturamento para L-BTC (Bitcoin na rede Liquid) como reserva de valor de longo prazo.
- **Gaste no mundo todo.** Use o cartão Kast para gastar seus fundos cripto em qualquer estabelecimento que aceita cartão.
- **Saque quando quiser.** Precisa de dinheiro no banco? Converta seus DePix de volta para reais e receba via Pix.

Para um pequeno empresário ou autônomo, ter essas opções é ter verdadeira liberdade financeira. Você decide o que fazer com cada centavo do seu faturamento, sem pedir permissão a nenhum banco.

## Relatórios de vendas integrados

Gerir um negócio exige controle. O DePix App oferece **relatórios de vendas** completos direto no aplicativo:

- **Filtre por período:** veja as vendas de hoje, da semana, do mês ou de qualquer intervalo personalizado.
- **Busque transações:** encontre vendas específicas rapidamente.
- **Exporte para PDF e CSV:** envie relatórios para seu contador ou use em planilhas para controle financeiro.

Tudo isso direto do celular, sem precisar de sistemas extras. Para quem precisa prestar contas ou simplesmente quer acompanhar o desempenho do negócio, os relatórios do DePix resolvem.

## Passo a passo: como começar a aceitar pagamentos com DePix

Vamos ao prático. Em menos de 10 minutos você pode estar pronto para aceitar pagamentos:

**Passo 1 — Crie sua conta.** Acesse [depixapp.com](https://depixapp.com/) e cadastre-se. Você só precisa de um e-mail. Não pedem CNPJ, não pedem documento, não pedem selfie. Um minuto e pronto.

**Passo 2 — Configure sua carteira Liquid.** Baixe a carteira SideSwap (disponível para Android e iOS) e crie sua carteira. Copie seu endereço Liquid.

**Passo 3 — Cadastre o endereço no DePix App.** Cole seu endereço Liquid nas configurações do DePix App. É para esse endereço que seus DePix serão enviados.

**Passo 4 — Comece a vender.** No modo lojista, gere um QR code Pix com o valor da venda. Mostre para o cliente. Ele paga via Pix normalmente. Você recebe DePix na sua carteira.

Simples assim. Sem contrato, sem aprovação, sem espera.

## Sobre o prazo de processamento de 24 horas

Uma informação importante: os depósitos DePix levam **até 24 horas (D+1)** para chegar na sua carteira¹. Isso não é um atraso — é uma medida de segurança.

Esse prazo existe para proteger tanto o comerciante quanto o sistema contra fraudes e golpes com Pix. Se você quer entender em detalhes como essa proteção funciona, leia nosso artigo sobre [como o DePix App protege contra golpes](/posts/como-depix-protege-contra-golpes/).

Na prática, o prazo D+1 é semelhante ao que muitas maquininhas oferecem para vendas no débito. E é muito melhor do que os 30 dias de prazo para vendas parceladas no crédito.

## Programa de afiliados

O DePix App também oferece um **programa de afiliados**. Se você indicar outros comerciantes ou usuários, pode ganhar uma comissão sobre as transações deles. É uma forma extra de renda para quem já usa o sistema e quer divulgar para outros empresários e autônomos.

## Referências

1. [DePix App — Taxas, limites e modo lojista](https://depixapp.com/)
2. [Banco Central do Brasil — Estatísticas de arranjos de pagamento](https://www.bcb.gov.br/estabilidadefinanceira/estatisticasarranjos)

## Sua privacidade financeira começa aqui

Pix tem praticidade — DePix tem privacidade. Pare de expor cada transação ao sistema bancário e comece a usar dinheiro digital que só você controla. Abra sua conta PJ cripto em minutos e livre-se da burocracia bancária. Acesse [depixapp.com](https://depixapp.com/).
